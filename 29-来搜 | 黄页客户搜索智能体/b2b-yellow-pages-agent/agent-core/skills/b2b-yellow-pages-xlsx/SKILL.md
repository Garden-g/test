---
name: b2b-yellow-pages-xlsx
displayName: "黄页客户搜索"
displayDescription: "从海外黄页和本地目录搜索企业客户并生成表格名单"
description: 黄页客户搜索 XLSX 技能。Use when the user wants to find overseas B2B leads from Yellow Pages, local business directories, phone/address directories, category directories, or country-specific listing sites by product/category, customer type, and country, then export a verified Excel workbook.
---

# 黄页客户搜索

把“产品 + 客户类型 + 国家/地区”从Yellow Pages、本地商家目录、行业目录和电话地址类页面里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "\"pet supplies\" \"importer\" \"United States\" \"yellow pages\" -alibaba -made-in-china -1688 -aliexpress -amazon -ebay -china supplier -classifieds -jobs",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从Yellow Pages、本地商家目录、行业目录和电话地址类页面里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
"{product}" "{customer_type}" "{country}" "yellow pages"
"{product}" "{customer_type}" "{country}" "business directory"
"{product}" "wholesale" "{country}" "phone" "address"
"{product}" "distributor" "{country}" "directory"
site:yellowpages.* "{product}" "{country}"
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-alibaba -made-in-china -1688 -aliexpress -amazon -ebay -china supplier -classifieds -jobs
```
...
| URL | 结果链接 |
| 来源类型 | Yellow Pages、本地商家目录、行业目录和电话地址类页面、官网、目录、社媒、新闻、行业来源、其他 |
...
- companies with phone, address, website, and category match
- wholesalers, distributors, retailers, repair/service companies only when relevant to B2B sales
- directory records that can be cross-checked with official websites

候选剔除：

- personal services unrelated to the product
- classified ads, jobs, marketplace listings, duplicated directory mirrors
...
3. official website from listing
4. Contact / About pages from the official website
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-yellow-pages-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
黄页数据常有过期信息。电话和地址可交付，但邮箱/联系人必须有公开来源才写入。

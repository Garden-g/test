---
name: b2b-instagram-development-xlsx
displayName: "图片社媒客户开发"
displayDescription: "用图片社交平台公开搜索寻找外贸企业客户并生成表格名单"
description: Instagram 客户开发 XLSX 技能。Use when the user wants to find overseas B2B brands, retailers, distributors, boutiques, shops, or social-commerce buyers from Instagram public search results by product/category, customer type, and country, then export a verified Excel workbook.
---

# Ins客户开发

把“产品 + 客户类型 + 国家/地区”从Instagram 公开主页和主页外链里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "site:instagram.com \"pet supplies\" \"United States\" \"importer\" -/p/ -/reel/ -/reels/ -/explore/ -/tags/ -hashtag -fanpage -alibaba -made-in-china -1688 -aliexpress -china supplier",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从Instagram 公开主页和主页外链里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
site:instagram.com "{product}" "{country}" "{customer_type}"
site:instagram.com "{product}" "{country}" "wholesale"
site:instagram.com "{product}" "{country}" "distributor"
site:instagram.com "{product}" "{country}" "store"
site:instagram.com "{product}" "{country}" "official"
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-/p/ -/reel/ -/reels/ -/explore/ -/tags/ -hashtag -fanpage -alibaba -made-in-china -1688 -aliexpress -china supplier
```
...
| URL | 结果链接 |
| 来源类型 | Instagram 公开主页和主页外链、官网、目录、社媒、新闻、行业来源、其他 |
...
- brand, retailer, distributor, boutique, or store profiles with business identity
- profiles with website, email, phone, address, or product catalog link
- accounts whose content and bio match the product and target country

候选剔除：

- single posts, reels, tags, explore pages, fan pages, influencers without buying/company context
- China supplier profiles unless requested
...
1. Instagram public profile result
2. bio link / official website if visible
3. linked website home page
4. Contact / About / Products pages from the linked website
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-instagram-development-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
Instagram 只做公开主页开发线索，不登录、不私信。帖子和 Reels 只能辅助判断活跃度，不作为客户主体。

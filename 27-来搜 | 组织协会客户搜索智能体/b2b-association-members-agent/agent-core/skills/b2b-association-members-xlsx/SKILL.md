---
name: b2b-association-members-xlsx
displayName: "组织协会客户搜索"
displayDescription: "从协会会员目录搜索海外企业客户并生成表格名单"
description: 组织协会客户搜索 XLSX 技能。Use when the user wants to find overseas B2B customer leads from industry associations, chambers of commerce, member directories, trade organizations, buyer groups, or professional organization member lists by product/category, customer type, and country, then export a verified Excel workbook.
---

# 组织协会客户搜索

把“产品 + 客户类型 + 国家/地区”从行业协会、商会、会员目录、member list 和 directory 页面里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "\"pet supplies\" \"United States\" association member directory -alibaba -made-in-china -1688 -aliexpress -china supplier -membership fee -join us -events only",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从行业协会、商会、会员目录、member list 和 directory 页面里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
"{product}" "{country}" association member directory
"{product}" "{country}" trade association members
"{product}" "{country}" chamber of commerce member directory
"{product}" "{customer_type}" "{country}" members
"{product}" "{country}" industry association directory
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-alibaba -made-in-china -1688 -aliexpress -china supplier -membership fee -join us -events only
```
...
| URL | 结果链接 |
| 来源类型 | 行业协会、商会、会员目录、member list 和 directory 页面、官网、目录、社媒、新闻、行业来源、其他 |
...
- member companies matching product/customer type/country
- importers, distributors, retailers, brands, wholesalers, and service/channel firms with B2B relevance
- member records with official company website or contact details

候选剔除：

- the association itself unless it is a target account
- events-only pages and membership recruitment pages without member list
...
1. association member directory
2. member company profile
3. member official website
4. About / Contact / Products pages from member company
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-association-members-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。

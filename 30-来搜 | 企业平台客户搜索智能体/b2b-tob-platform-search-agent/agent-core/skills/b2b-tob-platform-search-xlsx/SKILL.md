---
name: b2b-tob-platform-search-xlsx
displayName: "企业平台客户搜索"
displayDescription: "从海外企业平台和行业目录搜索客户并生成表格名单"
description: ToB 平台客户搜索 XLSX 技能。Use when the user wants to find overseas B2B customer leads from business directories and ToB platforms such as Thomasnet, Europages, Kompass, GlobalSpec, industry portals, or supplier/buyer directories while filtering out China suppliers and marketplace noise, then export a verified Excel workbook.
---

# ToB平台客户搜索

把“产品 + 客户类型 + 国家/地区”从Thomasnet、Europages、Kompass、GlobalSpec 等海外 ToB 平台和行业目录里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "site:thomasnet.com \"pet supplies\" \"United States\" distributor -alibaba -made-in-china -1688 -aliexpress -globalsources -china supplier -chinese manufacturer -exporter china",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从Thomasnet、Europages、Kompass、GlobalSpec 等海外 ToB 平台和行业目录里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
site:thomasnet.com "{product}" "{country}" distributor
site:europages.com "{product}" "{country}" "{customer_type}"
site:kompass.com "{product}" "{country}" "{customer_type}"
site:globalspec.com "{product}" "{country}" distributor
"{product}" "{customer_type}" "{country}" "company profile" "B2B"
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-alibaba -made-in-china -1688 -aliexpress -globalsources -china supplier -chinese manufacturer -exporter china
```
...
| URL | 结果链接 |
| 来源类型 | Thomasnet、Europages、Kompass、GlobalSpec 等海外 ToB 平台和行业目录、官网、目录、社媒、新闻、行业来源、其他 |
...
- overseas distributors, wholesalers, importers, retailers, brands, system integrators, or channel companies
- profiles with website/contact and country/category evidence
- platform records that can be cross-checked against the official company site

候选剔除：

- Chinese suppliers, pure manufacturers when user wants buyers, marketplace product listings without company identity
- duplicate directory mirrors
...
1. ToB platform company profile
2. company website from profile
3. About / Contact / Products pages
4. platform category page when it identifies the company
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-tob-platform-search-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
ToB 平台上供应商和客户会混在一起。默认面向外贸公司找海外买家/渠道，必须过滤中国供应商和纯供应商结果。

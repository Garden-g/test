---
name: b2b-facebook-development-xlsx
displayName: "脸书客户开发"
displayDescription: "用脸书公开搜索寻找外贸企业客户并生成表格名单"
description: Facebook 客户开发 XLSX 技能。Use when the user wants to find overseas B2B company pages, local retailers, distributors, wholesalers, brands, or active social business pages from Facebook public search results, then export a verified Excel workbook.
---

# FB客户开发

把“产品 + 客户类型 + 国家/地区”从Facebook 公开主页和主页外链里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "site:facebook.com \"pet supplies\" \"United States\" \"importer\" -groups -posts -events -marketplace -watch -reel -people -share -login -alibaba -made-in-china -1688 -aliexpress -china supplier",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从Facebook 公开主页和主页外链里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
site:facebook.com "{product}" "{country}" "distributor"
site:facebook.com "{product}" "{country}" "retailer"
...
```text
-groups -posts -events -marketplace -watch -reel -people -share -login -alibaba -made-in-china -1688 -aliexpress -china supplier
```
...
| URL | 结果链接 |
| 来源类型 | Facebook 公开主页和主页外链、官网、目录、社媒、新闻、行业来源、其他 |
...
- Facebook Pages with business name, country/location, product/category match, and contact clues
- pages that point to independent websites or show active business operations
- retail, distribution, wholesale, and brand pages relevant to the product

候选剔除：

- groups, posts, events, marketplace listings, watch/reel URLs, people pages, fan communities
...
1. Facebook public page result
2. page website link when visible
3. official website home page
4. About / Contact / Products pages from the linked website
  ...
  "leads": [
    {
      ...
      "social_links": ["https://www.facebook.com/example"],
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-facebook-development-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
Facebook 只做公开主页搜索，不登录、不私信、不加入群组。主页可作为线索，最终优先用官网或公开联系方式交付。

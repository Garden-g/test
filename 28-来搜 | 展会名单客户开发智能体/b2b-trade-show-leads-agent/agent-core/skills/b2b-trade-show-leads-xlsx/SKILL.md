---
name: b2b-trade-show-leads-xlsx
displayName: "展会名单客户开发"
displayDescription: "从展会参展商和官方目录搜索客户并生成可核验表格名单"
description: 展会名单客户开发 XLSX 技能。Use when the user wants to find overseas B2B customer leads from trade show exhibitor lists, exhibitor directories, brand lists, floor plans, show guides, or event websites by product/category, customer type, and country, then export a verified Excel workbook.
---

# 展会名单客户开发

把“产品 + 客户类型 + 国家/地区”从展会官网、参展商目录、show guide、floor plan 和品牌列表里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "\"pet supplies\" \"United States\" exhibitor list -alibaba -made-in-china -1688 -aliexpress -china supplier -booth builder -event services -ticket",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从展会官网、参展商目录、show guide、floor plan 和品牌列表里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
"{product}" "{country}" exhibitor list
"{product}" "{country}" exhibitor directory
"{product}" "{country}" show guide exhibitors
"{product}" "{country}" floor plan brands
"{product}" trade show "{country}" "{customer_type}"
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-alibaba -made-in-china -1688 -aliexpress -china supplier -booth builder -event services -ticket
```
...
| URL | 结果链接 |
| 来源类型 | 展会官网、参展商目录、show guide、floor plan 和品牌列表、官网、目录、社媒、新闻、行业来源、其他 |
...
- official exhibitor records with company name and category
- brands, distributors, wholesalers, retailers, importers, and channel companies matching the product
- companies with website/contact details or booth/category evidence

候选剔除：

- exhibition service vendors, booth builders, media pages, ticket pages, event organizers unless they are the target
- Chinese suppliers unless the user wants China
...
1. official exhibitor directory
2. exhibitor profile page
3. exhibitor website
4. About / Contact / Products pages
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-trade-show-leads-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
优先展会官方来源。第三方转录名单只能作辅助证据，必须标注可信度。

---
name: b2b-linkedin-customer-list-xlsx
displayName: "领英客户名单"
displayDescription: "用领英公开搜索寻找外贸企业客户并生成表格名单"
description: 领英客户名单 XLSX 技能。Use when the user wants to find overseas B2B customer companies, buyers, importers, wholesalers, distributors, brands, retailers, or decision makers from LinkedIn public search results by product/category, customer type, and country, then export a verified Excel workbook.
---

# 领英客户名单

把“产品 + 客户类型 + 国家/地区”从LinkedIn 公开公司页和必要的公开个人页里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "site:linkedin.com/company \"pet supplies\" \"United States\" \"importer\" -jobs -learning -pulse -school -showcase -login -signup -alibaba -made-in-china -1688 -aliexpress -china supplier",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从LinkedIn 公开公司页和必要的公开个人页里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
site:linkedin.com/company "{product}" "{country}" "{customer_type}"
site:linkedin.com/company "{product}" "{country}" "distributor" OR "wholesaler"
site:linkedin.com/company "{product}" "{country}" "import"
site:linkedin.com/in "{product}" "{country}" "Purchasing Manager"
site:linkedin.com/in "{product}" "{country}" "Sourcing Manager"
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-jobs -learning -pulse -school -showcase -login -signup -alibaba -made-in-china -1688 -aliexpress -china supplier
```
...
| URL | 结果链接 |
| 来源类型 | LinkedIn 公开公司页和必要的公开个人页、官网、目录、社媒、新闻、行业来源、其他 |
...
- LinkedIn company pages with country and product/customer-type evidence
- public profiles tied to procurement, sourcing, buying, owner, founder, product, category, or distribution roles
- companies with an independent website or clear B2B business description

候选剔除：

- job posts, learning pages, Pulse articles, school pages, login/signup pages
- personal profiles without company context
- Chinese suppliers unless the user explicitly wants China
...
1. LinkedIn company public page
2. company website from LinkedIn snippet or profile
3. About / Contact pages on the company website
4. public decision-maker pages only when company context is clear
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-linkedin-customer-list-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
LinkedIn 只做公开搜索和公开页面证据抓取，不登录、不私信、不尝试绕过访问限制。个人页只能作为联系人线索，最终客户主体必须是公司。

---
name: b2b-maps-development-xlsx
displayName: "地图客户开发"
displayDescription: "用地图和本地商家公开搜索寻找企业客户并生成表格名单"
description: 地图客户开发 XLSX 技能。Use when the user wants to find overseas local B2B places, wholesalers, distributors, retail stores, warehouses, showrooms, factories, or offices from public Maps-style search results and local business pages, then export a verified Excel workbook.
---

# 地图客户开发

把“产品 + 客户类型 + 国家/地区”从Google Maps 公开搜索结果、本地商家目录和地图相关页面里整理成一份可交付的客户开发 `.xlsx`。核心原则是：先问清需求，再用公开网页证据找候选公司，过滤中国供应商和无关平台，最后只交付有来源、有空值标记、不编造的名单。
...
```json
{
  "fieldName_3": {
    "payload": {
      "query": "site:google.com/maps \"pet supplies\" \"importer\" \"United States\" -alibaba -made-in-china -1688 -aliexpress -amazon -ebay -china supplier -manufacturer directory",
      "api": "google"
    }
  }
}
...
用中文复述目标和来源，例如：“我会从Google Maps 公开搜索结果、本地商家目录和地图相关页面里找美国宠物用品进口商，优先保留有公司主体、官网、联系方式和产品匹配证据的公司。”
...
```text
site:google.com/maps "{product}" "{customer_type}" "{country}"
"{product}" "{customer_type}" "{country}" "Google Maps"
"{product}" "{customer_type}" "{country}" "near me"
"{product}" "wholesale" "{country}" "address" "phone"
"{product}" "distributor" "{country}" "address" "phone"
```

默认追加排除词，减少平台、中国供应商和无关页面噪声：

```text
-alibaba -made-in-china -1688 -aliexpress -amazon -ebay -china supplier -manufacturer directory
```
...
| URL | 结果链接 |
| 来源类型 | Google Maps 公开搜索结果、本地商家目录和地图相关页面、官网、目录、社媒、新闻、行业来源、其他 |
...
- businesses with address, phone, website, and category/location match
- stores, warehouses, distributors, wholesale offices, showrooms, and local B2B places
- companies whose website confirms the product category

候选剔除：

- consumer-only services unrelated to the product
- closed places when visible
...
1. Maps result pages visible from search snippets
2. business official website
3. local directory profile
4. Contact / Location / Store pages
...
`SKILL_INSTALL_PATH` 使用读取 `$b2b-maps-development-xlsx` 时返回的 `install_path`；不要硬编码开发机路径。
...
这是公开搜索路线，不替代现有 Apify Google Maps Actor。不要使用需要地图账号登录或付费抓取的能力。
